    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('rate limit') || 
          ...
          throw new Error('ไม่พบผู้ใช้ในระบบ กรุณาลงทะเบียนใหม่');
        } else if (error.status === 422) {
          throw new Error('ข้อมูลไม่ถูกต้อง กรุณาตรวจสอบอีเมลและลองใหม่');
        } else {
          throw new Error(`เกิดข้อผิดพลาด: ${error.message || 'ไม่สามารถส่งอีเมลได้'}`);
        }
      }
  ...
  const verifyEmailWithToken = async (token: string): Promise<EmailVerificationResult> => {
    ...
    try {
      ...
      try {
        ...
        console.log('⚠️ Magic link verification failed:', magicLinkError);
      }
      
      // Method 2: Try as email confirmation token (fallback)
      if (!verificationResult || verificationResult.error) {
        console.log('🧪 Attempting email confirmation verification...');
        verificationResult = await supabase.auth.verifyOtp({
          token_hash: token,
        ...
        if (!verificationResult.error) {
          console.log('✅ Email confirmation verification successful');
        }
      }
      
      // Method 3: Try as signup confirmation (another fallback)
      if (!verificationResult || verificationResult.error) {
        console.log('🧪 Attempting signup confirmation verification...');
        verificationResult = await supabase.auth.verifyOtp({
          token_hash: token,
        ...
        if (!verificationResult.error) {
          console.log('✅ Signup confirmation verification successful');
        }
      }
      ...
      if (error) {
        ...
        if (error.message.includes('expired')) {
          ...
          throw new Error('ลิงก์ยืนยันไม่ถูกต้อง กรุณาตรวจสอบลิงก์จากอีเมลของคุณ');
        } else {
          throw new Error(`การยืนยันล้มเหลว: ${error.message}`);
        }
      }
      ...
      if (data.user) {
        ...
        if (updateError) {
          ...
          console.log('✅ Profile table updated - email_verified: true');
        }
